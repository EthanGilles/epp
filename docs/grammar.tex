...
\textbf{Keywords}
\begin{itemize}
  \item 'space' $\to$ int\_lit with value 32 (ASCII code for space)
  \item 'newline' $\to$ int\_lit with value 10 (ASCII code for \textbackslash n)
  \item 'true' $\to$ int\_lit with value 1 
  \item 'false' $\to$ int\_lit with value 0
  \item \emoji{folded-hands} $\to$ please
\end{itemize}
...
\begin{align*}
  ...
  \begin{cases}
    ...
    \text{for} ([\text{SetStmtID}]; [\text{Expr}]; [\text{ResetStmt}])[\text{Scope}] \\
    [\text{Scope}] \\
  \end{cases} \\ 
  [\text{SetStmt}] &\to 
  \begin{cases}
  ...
  \end{cases} \\
  [\text{SetStmtID}] &\to \text{set} \; [\text{ID}] = [\text{Expr}]; \\
  [\text{SetListID}] &\to \text{set} \; [\text{ListID}] = [\text{List}]; \\
\end{align*}
...
\begin{align*}
  ...
  \text{id\_lit} &\to [\text{a-zA-Z}][\text{0-9a-zA-Z\_}]^* \\
\end{align*}
